...
- Cover success, Provider failures, validation failures, time/iteration limits, no-progress loops, path escape, patch conflicts, approval denial, output truncation, process-tree timeout cleanup, and pre-existing Git changes.
...
- Flag new V0.1 features involving DeepSeek, multi-agent orchestration, RAG, MCP, memory, GUI, Git writes, cloud execution, or strong sandbox claims unless the specs were revised and re-approved.
