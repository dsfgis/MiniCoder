...
- `FACT-006`：用户于 2026-08-17 要求将产品名称从 `Mini Codex`（含非标准写法 `miniCodex`）改为 `Mini Coder`。
...
产品的人类可读名称必须统一为 `Mini Coder`，公开 CLI usage name、Maven artifactId、可执行 JAR、分发 ZIP 和分发包内 JAR 必须统一使用机器标识 `mini-coder`。内部 Java package `dev.minicodex` 与既有规格目录 `specs/mini-codex-v0.1/` 保持不变，它们是稳定的内部标识，不作为对外品牌展示。默认不继续生成旧名 `mini-codex` 的兼容制品。

- `AC-042` Given README、架构文档、变更记录、项目规则、Agent 指令、规格标题和 CLI 帮助，When 检索人类可读产品名称，Then 对外品牌统一显示为 `Mini Coder`，旧品牌只允许出现在改名说明中。
...
- `AC-045` Given 完成改名后的源码树，When 扫描 `Mini Codex`、`miniCodex` 与 `mini-codex`，Then 除改名说明、内部 package/目录路径及历史证据外，不存在旧对外品牌或发行标识；保留项均有明确用途。
