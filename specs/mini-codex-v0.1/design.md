...
对外展示和发行标识采用以下唯一映射；不通过批量重命名内部 package 或规格目录来扩大改名风险。（`REQ-017`）

| 类别 | 改名后标识 | 兼容策略 |
|---|---|---|
| 人类可读产品名 | `Mini Coder` | 旧品牌仅保留在迁移/变更说明中 |
| CLI usage name | `mini-coder` | 不保留旧 usage name |
| Maven artifactId | `mini-coder` | 不生成旧 artifactId 的兼容制品 |
...
| Java package | `dev.minicodex` | 保持稳定；不属于对外品牌 |
| 规格目录 | `specs/mini-codex-v0.1/` | 保持稳定，避免文档链接迁移 |

CLI 单元测试断言 command name、版本文本和示例；打包验证断言制品名、ZIP 内容以及旧制品不存在。文档扫描允许旧名只出现在改名说明、内部标识和历史证据中，并要求人工确认每个保留项的用途。（`AC-042`–`AC-045`）

## 14. 迁移、发布与回滚

这是新建项目，无既有数据迁移。V0.1 发布物为可执行 JAR/分发包、README、安全边界和示例配置。产品改名通过一次干净构建切换到 `mini-coder` 制品；旧 `mini-codex` 制品不作为兼容产物继续发布。若需要回滚品牌变更，必须整体回退名称、CLI 测试、打包配置和文档，避免混合命名。（`REQ-017`）
