...
- **交付物**：将人类可读品牌更新为 `Mini Coder`；将 Picocli usage name、Maven artifactId、fat JAR、分发 ZIP 和 ZIP 内 JAR 更新为 `mini-coder`；更新 CLI/打包测试、README、架构说明、变更记录、项目规则、Agent 指令和验证证据；保留内部 `dev.minicodex` package 与 `specs/mini-codex-v0.1/` 目录。
...
- **预期结果**：全量离线测试退出 0；CLI、文档和新制品统一使用 `Mini Coder` / `mini-coder`；ZIP 内含 `mini-coder.jar`；干净构建不产生旧名制品；内部 package/规格路径不变；历史证据不被伪造或改写。
...
| `REQ-017` | `AC-042`–`AC-045` | `TASK-017` |

覆盖结论：当前 17 个需求和 45 个验收标准均至少映射到一个实施任务；没有任务引入 V0.1 范围外的多 Agent、RAG、MCP、长期记忆、Git 写入或强沙箱承诺。
